...
for start, end in connections:
    ...
    caves.add(end)
caves = list(caves)
connectivity = np.zeros((len(caves), len(caves)), dtype='int')
for start, end in connections:
    connectivity[caves.index(start), caves.index(end)] = 1
    connectivity[caves.index(end), caves.index(start)] = 1

def num_paths(start_cave, visited, twice):
    if start_cave == 'end':
        return 1

    if start_cave[0] in string.ascii_lowercase:
        visited.add(start_cave)

    n = 0
    for i in np.flatnonzero(connectivity[caves.index(start_cave)]):
        if caves[i] not in visited:
            n += num_paths(caves[i], visited=set(visited), twice=twice)
        elif twice is None and caves[i] not in ['start', 'end']:
            n += num_paths(caves[i], visited=set(visited), twice=caves[i])
    return n
...
## Day 14 alternate implementation
# Read the template and the rules from the puzzle input
#with open('day14_input.txt') as f:
#    template = f.readline().strip()
#    f.readline()
#    rules = dict()
#    for line in f:
#        pair, between = line.strip().split(' -> ')
#        rules[pair] = ((pair[0] + between), (between + pair[1]))
#
## Compile a list of all possible pairs (n=100)
#all_pairs = sorted(rules.keys())
#
## Construct a 100 x 100 transformation matrix from pair -> new pairs
#trans = np.zeros((len(all_pairs), len(all_pairs)), dtype='int64')
#for from_pair, (to_pair1, to_pair2) in rules.items():
#    trans[all_pairs.index(from_pair), all_pairs.index(to_pair1)] += 1
#    trans[all_pairs.index(from_pair), all_pairs.index(to_pair2)] += 1
#
## Construct a vector containing the initial pairs
#initial_pairs = np.zeros(len(all_pairs))
#for a, b, in zip(template[:-1], template[1:]):
#    initial_pairs[all_pairs.index(a + b)] += 1
#
## Combine all 40 steps into a single transformation matrix
#supertrans = np.linalg.matrix_power(trans, 40)
#
## Apply the transformation matrix
#res = initial_pairs @ supertrans
#
## Count the number of elements contained in the pairs
#counts = defaultdict(int)
#for (x, _), count in zip(all_pairs, res):
#    counts[x] += count
#counts[template[-1]] += 1
#part2_ans = int(np.ptp(list(counts.values())))
#print('Day 14, part 2:', int(np.ptp(list(counts.values()))))
...
def solve_grid(risk_level):
    ...
    for i in range(n_rows * n_cols):
        ...
        for neighbor in node + directions:
        ...
        # plt.clf()
        # plt.imshow(to_plot, cmap='magma', vmin=-1, vmax=15)
        # plt.pause(0.001)
        # to_plot[node] = -1
    else:
